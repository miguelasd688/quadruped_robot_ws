...
class PS5Controller(Node):
    ...
    def __init__(self):
        ...
        self.leg_calibration_publisher = self.create_publisher(Twist, 'leg_calibration', 10)
    ...
    def publishCommands(self):
        ...
        leg_calibration = Twist()
        leg_calibration.linear.x = self.inputs.calibration_increment_x
        leg_calibration.linear.y = self.inputs.calibration_increment_y
        leg_calibration.linear.z = self.inputs.calibration_increment_z
        self.leg_calibration_publisher.publish(leg_calibration)
    ...
    def publishStatus(self):
        ...
        status_msg.data = [
            ...
            int(self.inputs.calibration_action), # Calibration action
    ...
    def printInformation(self, latency):
        if (time.time() - self.last_time_print >= 0.1):
            ...
            self.get_logger().info(f'    · x: {self.inputs.calibration_increment_x}, y: {self.inputs.calibration_increment_y}, z: {self.inputs.calibration_increment_z}    | Confirm : {self.inputs.calibration_action}')
