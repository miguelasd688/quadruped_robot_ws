...
void displayUpdate() {
  ...
  if (PAGE == 0) {
    ...
    if (t - previousPlot >= float(timePlot) / float(maxValues) - 0.016) { // every 0.03s seconds, 30 hz
      updatePlot(t, previousPlot , Vin, timePlot , xini, yini, w, h, ndivX , ndivY, YELLOW);
      previousPlot = t;
    }
  }
...
void infoText0() {
  ...
  tft.setCursor(64, 15);
  tft.setTextColor(BLACK);
  tft.print(oloopTime / 1000, 1);
  tft.setCursor(64, 15);
  tft.setTextColor(MAGENTA);
  tft.print(loopTime / 1000, 1);
